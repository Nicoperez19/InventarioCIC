...
class UpdateUserRequest extends FormRequest
{
    ...
    {
        $run = $this->route('user');
        $ignoreRun = is_string($run) ? $run : (method_exists($run, 'getAttribute') ? $run->getAttribute('run') : null);

        return [
            'run' => ['required', 'string', 'max:255', 'regex:/^\\d{7,8}-[\\dK]$/', Rule::unique('users', 'run')->ignore($ignoreRun, 'run')],
            'nombre' => ['required', 'string', 'max:255'],
            'correo' => ['required', 'email', 'max:255', Rule::unique('users', 'correo')->ignore($ignoreRun, 'run')],
