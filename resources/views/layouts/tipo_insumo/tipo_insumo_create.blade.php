            ...
            <div class="flex items-center space-x-3 sm:space-x-4">
                ...
                <div class="min-w-0 flex-1">
                    <h2 class="text-lg sm:text-xl font-semibold leading-tight text-gray-800 truncate">
                        {{ __('Agregar nuevo tipo de insumo') }}
                    </h2>
                    <p class="text-xs sm:text-sm text-gray-600 mt-1 hidden sm:block">Crea un nuevo tipo de insumo en el sistema</p>
                </div>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="mx-auto space-y-6 max-w-7xl sm:px-6 lg:px-8">
            <div class="p-4 bg-white shadow sm:p-8 sm:rounded-lg">
                <form method="POST" action="{{ route('tipo-insumos.store') }}" class="space-y-6">
                    @csrf
                    ...
                    <div class="mb-6">
                        ...
                        <p class="text-sm text-gray-500 mt-1">Define el nombre, descripción y color del tipo de insumo</p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="descripcion" 
                                      class="w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-400 focus:border-primary-400 transition-colors" 
                                      rows="3" 
                                      placeholder="Descripción detallada del tipo de insumo...">{{ old('descripcion') }}</textarea>
                            @error('descripcion')
                                <div class="flex items-center mt-2 text-sm text-red-600">
                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    ...
                    <div class="flex items-center space-x-4">
                        <label class="flex items-center">
                            <input type="radio" name="activo" value="1" {{ old('activo', '1') == '1' ? 'checked' : '' }} 
                                   class="h-4 w-4 text-primary-400 focus:ring-primary-400 border-gray-300">
                            <span class="ml-2 text-sm text-gray-700">Activo</span>
                        </label>
                        <label class="flex items-center">
                            <input type="radio" name="activo" value="0" {{ old('activo') == '0' ? 'checked' : '' }} 
                                   class="h-4 w-4 text-primary-400 focus:ring-primary-400 border-gray-300">
