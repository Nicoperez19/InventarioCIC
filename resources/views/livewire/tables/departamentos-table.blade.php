        ...
        <table class="w-full table-fixed divide-y divide-neutral-200">
            ...
            <tbody class="bg-white divide-y divide-neutral-200">
                @forelse($departamentos as $departamento)
                    <tr wire:key="departamento-{{ $departamento->id_depto }}" class="hover:bg-light-cyan/10 transition-colors duration-150">
                        ...
                        <td class="w-4/12 px-3 sm:px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex items-center justify-end space-x-1 sm:space-x-3">
                                <!-- Botón Editar -->
                                <a href="{{ route('departamentos.edit', $departamento->id_depto) }}" 
                                ...
                                <!-- Botón Eliminar -->
                                <form action="{{ route('departamentos.destroy', $departamento->id_depto) }}" 
                                      method="POST" 
                                      class="inline" 
                                      onsubmit="return confirm('¿Estás seguro de que deseas eliminar el departamento \'{{ $departamento->nombre_depto }}\'? Esta acción no se puede deshacer.');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
