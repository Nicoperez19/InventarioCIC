    ...
    <div class="w-full overflow-x-auto">
        ...
        <thead class="bg-primary-100">
                <tr>
                    ...
                    <th class="w-7/12 px-3 sm:px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                        <div class="flex items-center space-x-1 pl-6">
                            <svg class="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                            </svg>
                            <span class="hidden sm:inline">Nombre del Departamento</span>
                            <span class="sm:hidden">Departamento</span>
                        </div>
                    </th>
                    <th class="w-4/12 px-3 sm:px-6 py-4 text-xs font-semibold text-gray-600 uppercase tracking-wider">
            ...
            <tbody class="bg-white divide-y divide-neutral-200">
                @forelse($departamentos as $departamento)
                    <tr wire:key="departamento-{{ $departamento->id_depto }}" class="hover:bg-secondary-50 transition-colors duration-150">
                        <td class="w-1/12 px-3 sm:px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-neutral-500">
                                {{ $departamento->id_depto }}
                            </div>
                        </td>
                        <td class="w-7/12 px-3 sm:px-6 py-4">
                            <div class="text-sm font-medium text-neutral-900 pl-6">{{ $departamento->nombre_depto }}</div>
                        </td>
                        <td class="w-4/12 px-3 sm:px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex items-center justify-end space-x-1 sm:space-x-3">
                                <!-- Botón Editar -->
                                <button type="button" 
                                        onclick="openEditModal('{{ $departamento->id_depto }}')"
                                ...
                                <!-- Botón Eliminar -->
                                <button type="button" 
                                        onclick="deleteDepartamento('{{ $departamento->id_depto }}', '{{ addslashes($departamento->nombre_depto) }}')"
                ...
                @empty
                    <tr>
                        <td class="px-3 sm:px-6 py-12 text-center" colspan="3">
                            <div class="flex flex-col items-center">
                                <svg class="w-12 h-12 text-neutral-500 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                                </svg>
                                <h3 class="text-lg font-medium text-neutral-900 mb-2">No hay departamentos</h3>
                                <p class="text-neutral-500">Comienza creando tu primer departamento para organizar tu estructura.</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
...
@push('scripts')
<script>
    function deleteDepartamento(idDepto, nombreDepto) {
        // Mostrar diálogo de confirmación
        if (window.confirmDelete) {
            window.confirmDelete(
                `¿Estás seguro de que deseas eliminar el departamento "${nombreDepto}"? Esta acción no se puede deshacer.`,
                function() {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            if (window.notifySuccess) {
                                const nombre = data.nombre_depto || nombreDepto;
                                window.notifySuccess(`Departamento "${nombre}" eliminado.`);
                            }
                            ...
                            // Mostrar error
                            if (window.notifyError) {
                                window.notifyError(data.message || 'Error al eliminar el departamento. Por favor, intenta nuevamente.');
                            } else {
                    ...
                    .catch(error => {
                        ...
                        if (window.notifyError) {
                            window.notifyError('Error al eliminar el departamento. Por favor, intenta nuevamente.');
                        } else {
                            alert('Error al eliminar el departamento. Por favor, intenta nuevamente.');
                        }
                    });
                }
            );
            ...
            if (confirm(`¿Estás seguro de que deseas eliminar el departamento "${nombreDepto}"? Esta acción no se puede deshacer.`)) {
                // Crear formulario y enviarlo
                const form = document.createElement('form');
                form.method = 'POST';
                form.action = `/departamentos/${encodeURIComponent(idDepto)}`;
